   ...
   public function chooseManufacturer($year) {
    ...
    public function carRegistration(Request $request, $id) {
        ...
        if(isset($request['images']) && !empty($request['images']) ){
            ...
            if(!is_dir(storage_path(). '/cars/'.$id.'/ids')){
                mkdir( storage_path(). '/cars/'.$id.'/ids');
            }*/
            $small_image_policy_card = Str::random(30).'-small.'.$extension;
            $original_car_registration_image = Str::random(30).'-big.'.$extension;
            \Storage::disk('s3')->put($dir.'/'.$small_image_policy_card, base64_decode($image));
            \Storage::disk('s3')->put($dir.'/'.$original_car_registration_image, base64_decode($image));
            /*\File::put($dir. '/' . $small_image_policy_card, base64_decode($image));
            \File::put($dir. '/' . $original_car_registration_image, base64_decode($image));*/
        }
    ...
    public function carProtection(Request $request, $id) {
        ...
        if(isset($request['images']) && !empty($request['images']) ){
            ...
            $image = str_replace(' ', '+', $image);
            $dir = storage_path(). '/cars/'.$id.'/insurance';
            if(!is_dir(storage_path(). '/cars')){
                mkdir( storage_path(). '/cars');
            }
            if(!is_dir(storage_path(). '/cars/'.$id)){
            ...
            if(!is_dir(storage_path(). '/cars/'.$id.'/insurance')){
                mkdir( storage_path(). '/cars/'.$id.'/insurance');
            }
            $image_policy_card = Str::random(30).'-small.'.$extension;
            $small_image_policy_card = Str::random(30).'-big.'.$extension;
            \File::put($dir. '/' . $image_policy_card, base64_decode($image));
            \File::put($dir. '/' . $small_image_policy_card, base64_decode($image));
        }
        $car_ins->car_id                            = $id;
    ...
    public function carPhotos(Request $request, $id) {
        if(isset($request['images']) && !empty($request['images'])){
            ...
            for ($i=0; $i < sizeof($request['images']); $i++) { 
                ...
                \File::put($dir.$original_image_path, base64_decode($image));
                \File::put($dir.$big_image_path, base64_decode($image));
                \File::put($dir.$small_image_path, base64_decode($image));
                \File::put($dir.$icon_image_path, base64_decode($image));
        ...
        return json_encode(['message' => $request['images']], 200);
    }
